...
tot_gene_random_list=[]
for chem,genelist in chem_to_genes_null.items():
    chem_gene_cleaned_random[chem] = set(genelist)
    tot_gene_random_list.extend(list(genelist))


#Let's define the background for future hypergeometric test (the total number of genes in our dataset)
tot_gene_random = len(set(tot_gene_random_list))

#Here we calculate all the statistical significance of the overlap between all possible exposure pairs with a 
#Fisher test
random_combination_list=list(it.combinations(chem_gene_cleaned_random.keys(),2))
exposure_random_graph_fisher={}
for i in random_combination_list:
    fisher_test=overlap_set_exposures(chem_gene_cleaned_random[i[0]],chem_gene_cleaned_random[i[1]],tot_gene_random)
    exposure_random_graph_fisher[i]=float(fisher_test)

with open('intermediate/exposure_random_graph_fisher.pickle', 'wb') as handle:
    pk.dump(exposure_random_graph_fisher, handle, protocol=pk.HIGHEST_PROTOCOL)

#Here, we define the unweighted projected graph
unweighted_random_exp_graph_significant=nx.Graph()
for i in random_combination_list:
    ji = overlap_jaccard(chem_gene_cleaned_random[i[0]],chem_gene_cleaned_random[i[1]])
    if ji>0:     #this ensures that there is a shared gene in the original bipartite graph
        unweighted_random_exp_graph_significant.add_edge(*i)
        unweighted_random_exp_graph_significant[i[0]][i[1]]['weight']=ji
        
nx.write_weighted_edgelist(unweighted_random_exp_graph_significant, "intermediate/unfiltered_weighted_random_exp_graph_significant.edgelist")
...
unfiltered_weighted_random_exp_graph_fisher_dict = {}                     #Let's extract the Fisher's exact test pvals from all pairs to keep only those
for e in unweighted_random_exp_graph_significant.edges():    #that are present in the EEN
    try:
        unfiltered_weighted_random_exp_graph_fisher_dict[e]=exposure_random_graph_fisher[(e[0],e[1])]
    except:
        unfiltered_weighted_random_exp_graph_fisher_dict[e]=exposure_random_graph_fisher[(e[1],e[0])]

exp_keys_list=list(unfiltered_weighted_random_exp_graph_fisher_dict.keys())
exp_pvalues_list=list(unfiltered_weighted_random_exp_graph_fisher_dict.values())
...
for alpha_t in fdr_threshold:
    ...
    for chem,fdr in exp_graph_fisher_adjusted.items():
        if float(fdr)<alpha_t:
            ji= overlap_jaccard(chem_gene_cleaned_random[chem[0]],chem_gene_cleaned_random[chem[1]])      #we are computing the odds ratio
...
with open('output/post_rev_comb_threshold_dict_edges_random_EEN_ji.pickle', 'wb') as handle:
    pk.dump(comb_threshold_dict_edges, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('output/post_rev_comb_threshold_dict_nodes_random_EEN_ji.pickle', 'wb') as handle:
