...
with open('intermediate/gene_hq_graph_fisher.pickle', 'rb') as handle:
    gene_hq_graph_fisher = pk.load(handle)

unweighted_gene_graph_significant = nx.read_weighted_edgelist("intermediate/unfiltered_weighted_hq_gene_graph_significant.edgelist")

unweighted_gene_graph_significant_fisher_pval_dict = {}
for e in unweighted_gene_graph_significant.edges():
    try:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_hq_graph_fisher[e]
    except:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_hq_graph_fisher[e[1],e[0]]
...
for chem,fdr in exp_graph_fisher_adjusted.items():
    if float(fdr)<0.01:
        ji= overlap_jaccard(gene_chem_hq_cleaned[chem[0]],gene_chem_hq_cleaned[chem[1]])      
...
backbone_exp_graph_significant = nx.Graph([(u, v, d) for u, v, d in weighted_exp_graph_significant_dif.edges(data=True) if d['alpha'] < 0.1])   #let's apply the backboning threshold approach

nx.write_weighted_edgelist(backbone_exp_graph_significant, "output/backbone_hq_gene_graph_significant_weighted.edgelist")