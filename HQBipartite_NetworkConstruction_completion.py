...
with open('intermediate/exposure_hq_graph_fisher.pickle', 'rb') as handle:
    exposure_hq_graph_fisher = pk.load(handle)

unweighted_exp_graph_significant = nx.read_weighted_edgelist("intermediate/unfiltered_weighted_hq_exp_graph_significant.edgelist")

unweighted_exp_graph_significant_fisher_pval_dict = {}
for e in unweighted_exp_graph_significant.edges():
    try:
        unweighted_exp_graph_significant_fisher_pval_dict[e] = exposure_hq_graph_fisher[e]
    except:
        unweighted_exp_graph_significant_fisher_pval_dict[e] = exposure_hq_graph_fisher[e[1],e[0]]
...
backbone_exp_graph_significant = nx.Graph([(u, v, d) for u, v, d in weighted_exp_graph_significant_dif.edges(data=True) if d['alpha'] < 0.5])   #let's apply the backboning threshold approach

nx.write_weighted_edgelist(backbone_exp_graph_significant, "output/backbone_hq_exp_graph_significant_weighted.edgelist")