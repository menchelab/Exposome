...
for chem,gene_list in chem_gene_hq.items():
    ...
    tot_chem_hg_list.append(chem)
    if len(n_genelist)>0:
        chem_gene_hq_cleaned[chem] = set(n_genelist)

gene_chem_hq_cleaned = {}
for chem,geneset in chem_gene_hq_cleaned.items():
    for g in geneset:
        try:
            gene_chem_hq_cleaned[g] |= {chem}
        except KeyError as e:
            gene_chem_hq_cleaned[g] = set([chem])


#Let's define the background for future hypergeometric test (the total number of genes in our dataset)
tot_exp_hq = len(set(tot_chem_hg_list))

#Here we calculate all the statistical significance of the overlap between all possible exposure pairs with a 
#Fisher test
hq_combination_list=list(it.combinations(gene_chem_hq_cleaned.keys(),2))
gene_hq_graph_fisher={}
for i in hq_combination_list:
    fisher_test=overlap_set_exposures(gene_chem_hq_cleaned[i[0]],gene_chem_hq_cleaned[i[1]],tot_exp_hq)
    gene_hq_graph_fisher[i]=float(fisher_test)

with open('intermediate/gene_hq_graph_fisher.pickle', 'wb') as handle:
    pk.dump(gene_hq_graph_fisher, handle, protocol=pk.HIGHEST_PROTOCOL)

#Here, we define the unweighted projected graph
unweighted_hq_gene_graph_significant=nx.Graph()
for i in hq_combination_list:
    ji = overlap_jaccard(gene_chem_hq_cleaned[i[0]],gene_chem_hq_cleaned[i[1]])
    if ji>0:     #this ensures that there is a shared gene in the original bipartite graph
        unweighted_hq_gene_graph_significant.add_edge(*i)
        unweighted_hq_gene_graph_significant[i[0]][i[1]]['weight']=ji
        
nx.write_weighted_edgelist(unweighted_hq_gene_graph_significant, "intermediate/unfiltered_weighted_hq_gene_graph_significant.edgelist")
...
unfiltered_weighted_hq_gene_graph_fisher_dict = {}                     #Let's extract the Fisher's exact test pvals from all pairs to keep only those
for e in unweighted_hq_gene_graph_significant.edges():    #that are present in the EEN
    try:
        unfiltered_weighted_hq_gene_graph_fisher_dict[e]=gene_hq_graph_fisher[(e[0],e[1])]
    except:
        unfiltered_weighted_hq_gene_graph_fisher_dict[e]=gene_hq_graph_fisher[(e[1],e[0])]

gene_keys_list=list(unfiltered_weighted_hq_gene_graph_fisher_dict.keys())
gene_pvalues_list=list(unfiltered_weighted_hq_gene_graph_fisher_dict.values())
...
for alpha_t in fdr_threshold:
    ...
    for chem,fdr in exp_graph_fisher_adjusted.items():
        if float(fdr)<alpha_t:
            ji= overlap_jaccard(gene_chem_hq_cleaned[chem[0]],gene_chem_hq_cleaned[chem[1]])      #we are computing the odds ratio
...
with open('output/post_rev_comb_threshold_dict_edges_hq_GGN_ji.pickle', 'wb') as handle:
    pk.dump(comb_threshold_dict_edges, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('output/post_rev_comb_threshold_dict_nodes_hq_GGN_ji.pickle', 'wb') as handle:
