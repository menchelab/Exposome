...
tot_exp_homomouse = len(set(tot_gene_homomouse_list))

with open('intermediate/gene_homomouse_graph_fisher.pickle', 'rb') as handle:
    gene_homomouse_graph_fisher = pk.load(handle)
...
unweighted_gene_graph_significant_fisher_pval_dict = {}
for e in unweighted_homomouse_gene_graph_significant.edges():
    try:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_homomouse_graph_fisher[e]
    except:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_homomouse_graph_fisher[e[1],e[0]]

exp_keys_list=list(unweighted_gene_graph_significant_fisher_pval_dict.keys())
exp_pvalues_list=list(unweighted_gene_graph_significant_fisher_pval_dict.values())
...
for chem,fdr in exp_graph_fisher_adjusted.items():
    if float(fdr)<0.001:
        ji= overlap_jaccard(gene_chem_homomouse_cleaned[chem[0]],gene_chem_homomouse_cleaned[chem[1]])      
        exp_graph_ji_significant[chem]=ji                                                     
    else:
        pass
...
backbone_exp_graph_significant = nx.Graph([(u, v, d) for u, v, d in weighted_exp_graph_significant_dif.edges(data=True) if d['alpha'] < 0.08])   #let's apply the backboning threshold approach

nx.write_weighted_edgelist(backbone_exp_graph_significant, "output/backbone_homomouse_gene_graph_significant_weighted.edgelist")