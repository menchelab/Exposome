...
#Fisher test
homomouse_combination_list=list(it.combinations(chem_gene_homo_mouse_cleaned.keys(),2))
exposure_homomouse_graph_fisher={}
for i in homomouse_combination_list:
    fisher_test=overlap_set_exposures(chem_gene_homo_mouse_cleaned[i[0]],chem_gene_homo_mouse_cleaned[i[1]],tot_gene_homomouse)
    exposure_homomouse_graph_fisher[i]=float(fisher_test)

with open('intermediate/exposure_homomouse_graph_fisher.pickle', 'wb') as handle:
    pk.dump(exposure_homomouse_graph_fisher, handle, protocol=pk.HIGHEST_PROTOCOL)

#Here, we define the unweighted projected graph
unweighted_homomouse_exp_graph_significant=nx.Graph()
for i in homomouse_combination_list:
    ji = overlap_jaccard(chem_gene_homo_mouse_cleaned[i[0]],chem_gene_homo_mouse_cleaned[i[1]])
    if ji>0:     #this ensures that there is a shared gene in the original bipartite graph
        unweighted_homomouse_exp_graph_significant.add_edge(*i)
        unweighted_homomouse_exp_graph_significant[i[0]][i[1]]['weight']=ji
        
nx.write_weighted_edgelist(unweighted_homomouse_exp_graph_significant, "intermediate/unfiltered_weighted_homomouse_exp_graph_significant.edgelist")
...
unfiltered_weighted_homomouse_exp_graph_fisher_dict = {}                     #Let's extract the Fisher's exact test pvals from all pairs to keep only those
for e in unweighted_homomouse_exp_graph_significant.edges():    #that are present in the EEN
    try:
        unfiltered_weighted_homomouse_exp_graph_fisher_dict[e]=exposure_homomouse_graph_fisher[(e[0],e[1])]
    except:
        unfiltered_weighted_homomouse_exp_graph_fisher_dict[e]=exposure_homomouse_graph_fisher[(e[1],e[0])]

exp_keys_list=list(unfiltered_weighted_homomouse_exp_graph_fisher_dict.keys())
exp_pvalues_list=list(unfiltered_weighted_homomouse_exp_graph_fisher_dict.values())
...
with open('output/post_rev_comb_threshold_dict_edges_homomouse_EEN_ji.pickle', 'wb') as handle:
    pk.dump(comb_threshold_dict_edges, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('output/post_rev_comb_threshold_dict_nodes_homomouse_EEN_ji.pickle', 'wb') as handle:
