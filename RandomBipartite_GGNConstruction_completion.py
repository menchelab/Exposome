...
tot_exp_homomouse = len(set(tot_gene_random_list))

with open('intermediate/gene_random_graph_fisher.pickle', 'rb') as handle:
    gene_random_graph_fisher = pk.load(handle)

random_combination_list=list(it.combinations([el for el in gene_chem_random_cleaned.keys() if str(el)!='nan'],2))
...
for e in unweighted_random_gene_graph_significant.edges():
    try:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_random_graph_fisher[e]
    except:
        unweighted_gene_graph_significant_fisher_pval_dict[e] = gene_random_graph_fisher[e[1],e[0]]
...
for chem,fdr in exp_graph_fisher_adjusted.items():
    if float(fdr)<0.00001:
        ji= overlap_jaccard(gene_chem_random_cleaned[chem[0]],gene_chem_random_cleaned[chem[1]])      
...
nx.write_weighted_edgelist(backbone_exp_graph_significant, "output/backbone_random_gene_graph_significant_weighted.edgelist")